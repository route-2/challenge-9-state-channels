...
function App(props) {
  ...
  */
  ...
  function recieveVoucher(clientAddress) {
    ...
    const arrayified = ethers.utils.arrayify(hashed);
    const signature = await userSigner.signMessage(arrayified);
    const address = voucher.currentTarget;
    const clientAddress = address.name;
    console.log(address)
    console.log(clientAddress)
    

    const signerAddr = ethers.utils.verifyMessage(arrayified,signature);
    console.log(signerAddr)
    if(signerAddr !== clientAddress){
      console.log("error not equal to client")
  
    }
    else{
      console.log("success")
      return voucher.data.updatedBalance;
    }
      ...
      */
      ...
      // if this voucher is the most valuable, and the client has a closing channel,
      // then we can close the channel and claim the funds.
      if (hasClosingChannel() && updatedBalance.eq(ethers.BigNumber.from(0))) {
        claimFunds();
      }
  ...
  const contractAddr = readContracts && readContracts.Streamer && readContracts.Streamer.address
  var totalETH = useBalance(localProvider,contractAddr)
  totalETH = totalETH && totalETH.toString()
